"""
Style Templates for Specter Theme System.
...
logger = logging.getLogger("specter.style_templates")

class ButtonStyleManager:
    """
    Unified button styling manager that ensures consistency across all button types.
    
    This class provides the single source of truth for button styling in Specter,
    ...
    def get_unified_button_style(colors, 
        ...
        if colors is None:
            ...
            if special_colors:
                ...
                if state == "toggle":
                    ...
                    hover_color = ColorUtils.lighten(colors.status_error, 0.15)
                    active_color = ColorUtils.darken(colors.status_error, 0.15)
                elif state == "success":
                    ...
                    hover_color = ColorUtils.lighten(colors.status_success, 0.15)
                    active_color = ColorUtils.darken(colors.status_success, 0.15)
                elif state == "warning":
                    # Warning state uses warning status color for caution actions
                    bg_color = colors.status_warning
                    text_color = colors.background_primary
                    hover_color = ColorUtils.lighten(colors.status_warning, 0.15)
                    active_color = ColorUtils.darken(colors.status_warning, 0.15)
                else:  # normal
        ...
        # Compute theme-derived border colors for hover/pressed/focus states
        if colors is not None:
            hover_border = colors.border_focus
            pressed_border = colors.primary
            focus_border = colors.border_focus
        else:
            hover_border = "rgba(255, 255, 255, 0.3)"
            pressed_border = "rgba(255, 255, 255, 0.5)"
            focus_border = "#4CAF50"

        # Compute checked state colors for checkable/toggle buttons
        if special_colors:
            checked_bg = special_colors.get("checked_bg", colors.primary if colors else "#4CAF50")
            checked_text = special_colors.get("checked_text", colors.text_primary if colors else "#ffffff")
        else:
            checked_bg = colors.primary if colors else "#4CAF50"
            checked_text = colors.text_primary if colors else "#ffffff"
        checked_hover = ColorUtils.lighten(checked_bg, 0.15)
        ...
        {widget_selector}:checked {{
            background-color: {checked_bg};
            color: {checked_text};
            border: 1px solid {checked_bg};
        }}
        {widget_selector}:checked:hover {{
...
class StyleTemplates:
    ...
    in the Specter application. Each template is theme-aware and follows
