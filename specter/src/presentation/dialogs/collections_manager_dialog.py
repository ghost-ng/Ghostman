...
logger = logging.getLogger("specter.presentation.collections_manager")
...
class CollectionsManagerDialog(QDialog):
    ...
    def _apply_fallback_theme(self):
        """Apply fallback theme styling when theme system is not available.

        Attempts to derive colors from the current theme. Falls back to
        neutral dark defaults only if the theme system is completely unavailable.
        """
        # Try to get colors from the theme system even if StyleTemplates failed
        try:
            themes = get_improved_preset_themes()
            colors = themes.get(self.theme_name, next(iter(themes.values())))
        except Exception:
            colors = None
        ...
        if colors:
            ...
            interactive_disabled = colors.interactive_disabled
