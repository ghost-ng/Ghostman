"""
Enhanced System Tray for Specter.
...
logger = logging.getLogger("specter.system_tray")
...
class EnhancedSystemTray(QObject):
    ...
    def _init_tray_icon(self):
        ...
        # Dynamic tooltip based on selected avatar persona
        try:
            ...
            self.tray_icon.setToolTip(f"{avatar.name if avatar else 'Specter'} - AI Assistant")
        except Exception:
            self.tray_icon.setToolTip("Specter - AI Assistant")
    ...
    def _get_persona_name(self) -> str:
        """Get the current avatar persona name for tooltips."""
        try:
            from ...infrastructure.storage.settings_manager import settings
            from ...domain.models.avatar_personas import get_avatar, DEFAULT_AVATAR_ID
            avatar_id = settings.get('avatar.selected', DEFAULT_AVATAR_ID)
    ...
    def set_avatar_mode(self):
        ...
        if self.tray_icon:
            ...
            self.tray_icon.setToolTip(f"{name} - Avatar Mode (Active)")
    ...
    def set_tray_mode(self):
        ...
        if self.tray_icon:
            ...
            self.tray_icon.setIcon(icon)
            name = self._get_persona_name()
            self.tray_icon.setToolTip(f"{name} - Tray Mode")
