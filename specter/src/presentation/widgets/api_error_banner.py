"""
API Error Banner Widget for Specter.
...
logger = logging.getLogger("specter.api_error_banner")
    ...
    def _apply_theme_styling(self):
        ...
        try:
            if self.theme_manager and hasattr(self.theme_manager, 'current_theme'):
                ...
                # Attempt to get colors from global theme manager
                colors = None
                try:
            ...
            if not colors or not hasattr(self.settings_button, 'styleSheet') or not self.settings_button.styleSheet():
                ...
                primary_color = colors.primary if colors and hasattr(colors, 'primary') else '#5c6bc0'
                button_text_color = colors.text_primary if colors and hasattr(colors, 'text_primary') else '#ffffff'
                button_style = f"""
                    QPushButton {{
