"""
Modernized REPL styling using StyleTemplates and ColorSystem.

This module provides theme-aware HTML styling for REPL messages using the centralized
StyleTemplates system and ColorSystem instead of hardcoded colors.
"""
...
def get_styled_html(html_content, base_color, style='ai', color_scheme=None):
    ...
    # Fallback implementation with ColorSystem colors
...
def _get_styled_html_fallback(html_content, base_color, style='ai', color_scheme=None):
    """
    Fallback implementation using ColorSystem instead of hardcoded colors.
    
    This is used when StyleTemplates is not available or fails to load.
    """
    try:
        # Use ColorSystem for consistent theming
        from ...ui.themes.color_system import ColorSystem
        colors = ColorSystem()
        
        # Get theme-appropriate background colors
        code_bg = colors.with_opacity(colors.text_primary, 0.1)
        pre_bg = colors.with_opacity(colors.text_primary, 0.05)
        link_color = colors.info or colors.primary
        
    except Exception:
        # Ultimate fallback if ColorSystem fails
    ...
    # Container class
    container_class = "specter-user-input" if style == 'input' else "specter-ai-response"
    ...
    replacements = {
        '<code>': f'<code style="{code_font}background-color: {code_bg}; padding: 2px 4px; border-radius: 3px; color: {style_colors["code"]};">',
        '<pre>': f'<pre style="{code_font}background-color: {pre_bg}; padding: 8px; border-radius: 4px; border-left: 3px solid {base_color}; margin: 4px 0; overflow-x: auto;">',
