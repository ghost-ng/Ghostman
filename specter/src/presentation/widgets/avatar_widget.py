...
logger = logging.getLogger("specter.avatar_widget")
...
class AvatarWidget(SimpleAvatarArrowMixin, AvatarResizableMixin, QWidget):
    ...
    def _load_avatar(self):
        """Load the avatar image based on the selected persona."""
        # Try persona-specific avatar from assets/avatars/
        try:
            ...
            persona = get_avatar(avatar_id)
            if persona and persona.avatar_image:
                img_path = resolve_asset(f"avatars/{persona.avatar_image}")
                if img_path:
                    self.avatar_pixmap = QPixmap(str(img_path))
                    if not self.avatar_pixmap.isNull():
                        logger.info(f"Avatar loaded for persona '{persona.name}': {img_path}")
                        self._update_scaled_pixmap()
                        return
            # Persona exists but has no image — render colored circle with initial
            if persona and not persona.avatar_image:
                self._create_persona_fallback(persona)
                return
        except Exception as e:
    ...
    def _create_persona_fallback(self, persona):
        """Create a persona-colored circle with initial letter as fallback avatar."""
        from PyQt6.QtGui import QColor, QBrush, QPen, QFont as QGuiFont
        ...
        color = QColor(persona.color_primary)
        accent = QColor(persona.color_accent)
        ...
        font = QGuiFont("Segoe UI", 64, QGuiFont.Weight.Bold)
        painter.setFont(font)
        painter.drawText(
            QRectF(10, 10, size - 20, size - 20),
            Qt.AlignmentFlag.AlignCenter,
            persona.initial,
        )
        painter.end()

        logger.info(f"Persona fallback avatar created for '{persona.name}'")
        self._update_scaled_pixmap()
    ...
    def _show_context_menu(self, pos: QPoint):
        ...
        # Quit action
        quit_action = QAction(self._get_theme_icon("exit"), "Quit Specter", self)
    ...
    def _on_quit_clicked(self):
        ...
        logger.info("🚪 Quit Specter clicked by user")
