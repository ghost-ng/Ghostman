"""
Logging Configuration for Specter.
...
def _resolve_log_dir() -> Path:
    """Determine the unified log directory under AppData/Specter/logs.
    ...
    if os.name == 'nt':  # Windows
        ...
        if appdata:
            base_path = Path(appdata) / "Specter"
        else:
            base_path = Path.home() / "AppData" / "Roaming" / "Specter"
    else:
        # Fallback for other platforms - use XDG standard
        xdg_data_home = os.environ.get('XDG_DATA_HOME')
        if xdg_data_home:
            base_path = Path(xdg_data_home) / "specter"
        else:
            base_path = Path.home() / ".local" / "share" / "specter"
...
def cleanup_old_logs(log_dir: Path, retention_days: int) -> int:
    ...
    try:
        ...
        patterns = ['specter-*.log', 'specter-errors-*.log']
...
def setup_logging(debug: bool = False, log_dir: str | None = None, retention_days: int = 10) -> None:
    """
    Setup logging configuration for Specter with daily rotation.
    ...
    log_file = str(log_dir_path / 'specter.log')
    ...
    file_handler.suffix = '-%Y-%m-%d.log'  # Format: specter-YYYY-MM-DD.log
    ...
    error_file = str(log_dir_path / 'specter-errors.log')
    ...
    error_handler.suffix = '-%Y-%m-%d.log'  # Format: specter-errors-YYYY-MM-DD.log
    # Set proper extension matching pattern for the suffix format
    ...
    logging.getLogger("specter").setLevel(logging.DEBUG if debug else logging.INFO)
