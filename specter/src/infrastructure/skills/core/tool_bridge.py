...
This module acts as the translation layer between the Specter skills framework
...
Usage:
    >>> from specter.src.infrastructure.skills.core.tool_bridge import tool_bridge
...
logger = logging.getLogger("specter.skills.tool_bridge")


class ToolCallingBridge:
    """
    Stateless bridge that translates between the Specter skills framework
    ...
    def build_tool_awareness_prompt(self, registry: "SkillRegistry") -> str:
        ...
        lines = [
            ...
            "- ONLY call a tool when the user EXPLICITLY requests an action that requires it.",
            "- For greetings, small talk, 'test', 'hello', 'hi', casual conversation, opinions, coding help, creative writing, or general questions you can answer yourself — just respond normally. Do NOT call any tool.",
            "- If you are unsure whether a tool is needed, do NOT call it. Respond with text instead.",
            "- NEVER call the same tool multiple times for a single user message.",
            "",
            "When to use specific tools:",
            "- web_search: ONLY when the user explicitly says 'search for', 'look up', 'find info about', or asks a specific factual question you cannot answer.",
            "- outlook_email: ONLY when the user explicitly mentions email (draft, send, search inbox, reply, forward).",
            "- outlook_calendar: ONLY when the user explicitly mentions calendar, meetings, or scheduling.",
            "- docx_formatter: ONLY when the user asks to format, edit, fix, or reformat a .docx file.",
            "- screen_capture: ONLY when the user says 'take a screenshot' or 'capture my screen'.",
            "",
