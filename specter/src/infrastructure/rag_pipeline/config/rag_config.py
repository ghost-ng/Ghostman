...
logger = logging.getLogger("specter.rag_config")
...
class VectorStoreConfig:
    ...
    collection_name: str = "specter_documents"
    ...
    def __post_init__(self):
        """Set default persist directory to AppData/Roaming/Specter/db if not provided."""
        if not self.persist_directory:
            # Use AppData/Roaming/Specter/db as requested (case-sensitive for cross-platform)
            if os.name == 'nt':  # Windows
                appdata = os.getenv('APPDATA', os.path.expanduser('~\\AppData\\Roaming'))
                data_dir = os.path.join(appdata, 'Specter', 'db')
            else:  # Unix/Mac - use consistent capitalization
                data_dir = os.path.expanduser("~/.Specter/db")
        ...
        try:
            ...
            if os.name == 'nt':  # Windows
                ...
                if appdata:
                    fallback_dir = os.path.join(appdata, "Specter", "rag", "faiss_db")
                else:
                    fallback_dir = os.path.join(os.path.expanduser("~"), ".Specter", "rag", "faiss_db")
            else:  # Linux/Mac
                fallback_dir = os.path.join(os.path.expanduser("~"), ".Specter", "rag", "faiss_db")
            ...
            try:
                ...
                import tempfile
                self.persist_directory = os.path.join(tempfile.gettempdir(), "specter_faiss_temp")
                os.makedirs(self.persist_directory, exist_ok=True)
...
class DocumentLoadingConfig:
    ...
    user_agent: str = "Specter-RAG/1.0"
    ...
    def __post_init__(self):
        ...
        if not self.data_directory:
            ...
            if os.name == 'nt':  # Windows
                appdata = os.environ.get('APPDATA')
                if appdata:
                    self.data_directory = os.path.join(appdata, "Specter", "rag")
                else:
                    self.data_directory = os.path.expanduser("~/.Specter/rag")
            else:  # Linux/Mac
                self.data_directory = os.path.expanduser("~/.Specter/rag")
