"""
Resource path resolution utilities for Ghostman.
...
logger = logging.getLogger("ghostman.resource_resolver")


class ResourceResolver:
    """
    Centralized resource path resolver for Ghostman application.
    ...
    def resolve_help_file(self, filename: str = "index.html") -> Optional[Path]:
        ...
        possible_paths = [
            ...
            Path(sys.prefix) / "share" / "ghostman" / "help" / filename,
            # Bundled executable mode (PyInstaller)
            Path(getattr(sys, '_MEIPASS', Path.cwd())) / "ghostman" / "assets" / "help" / filename,
            # Relative to current working directory
            Path.cwd() / "ghostman" / "assets" / "help" / filename,
        ]
    ...
    def resolve_icon(self, icon_name: str, theme_suffix: str = "") -> Optional[Path]:
        ...
        possible_paths = [
            ...
            # Installed package mode
            Path(sys.prefix) / "share" / "ghostman" / "icons" / full_icon_name,
            # Bundled executable mode
            Path(getattr(sys, '_MEIPASS', Path.cwd())) / "ghostman" / "assets" / "icons" / full_icon_name,
            # Relative to current working directory
    ...
    def resolve_asset(self, asset_path: str) -> Optional[Path]:
        ...
        possible_paths = [
            ...
            # Installed package mode
            Path(sys.prefix) / "share" / "ghostman" / asset_path,
            # Bundled executable mode
            Path(getattr(sys, '_MEIPASS', Path.cwd())) / "ghostman" / "assets" / asset_path,
            # Relative to current working directory
            Path.cwd() / "ghostman" / "assets" / asset_path,
        ]
