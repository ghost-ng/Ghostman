...
The Ghostman skills system provides a extensible framework for adding functionality to the AI assistant. Skills are self-contained units that:
...
result = await manager.execute_skill(
    ...
    to="user@example.com",
    subject="Test Email",
    body="This is a test email from Ghostman",
    priority="high"
)
...
```python
from ghostman.src.infrastructure.skills.intent_classifier import IntentClassifier
...
```python
from ghostman.src.infrastructure.skills.interfaces import RecurrenceType
...
This completes the comprehensive usage guide for the Ghostman skills system interfaces!
